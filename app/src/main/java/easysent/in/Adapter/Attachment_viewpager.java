...
public class Attachment_viewpager extends RecyclerView.Adapter<Attachment_viewpager.ViewHolder> {
    ...
    public Attachment_viewpager(List<Chats> chats, Context context, Handler handler) {
        this.chats = chats;
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (item.getType().equalsIgnoreCase("I")) {
            ...
            if (file.exists() && file.isAbsolute()) {
                ...
                Glide.with(context).load(Constants.BASE_URL + "Attachment/" + item.getAttachment())
                        .thumbnail(0.02f)
                        ...
                        .into(binding.ivAttachment);
                CashImage2(Constants.BASE_URL + "Attachment/"+item.getAttachment(), item.getAttachment());
            }
        ...
        if (item.getType().equalsIgnoreCase("V")) {
            ...
            String path = Constants.BASE_URL + "Attachment/" + item.getAttachment();
            File file  = new File(CATCH_DIR2 + "/"+item.getAttachment());
            if (file.exists()){
